...
function createPreviewCard(card) {
  ...
  let date = `${d.getDate()}/${d.getUTCMonth()+1} - ${d.getFullYear()}`
...
function getPosts() {
  ...
  fetch("http://localhost:8888/mysite/wp-json/wp/v2/posts?_embed&per_page=100")
...
function getPostFromId() {
  ...
  fetch(`http://localhost:8888/mysite/wp-json/wp/v2/posts/${id}?_embed`)
    .then((response) => response.json())
    .then((data) => {
